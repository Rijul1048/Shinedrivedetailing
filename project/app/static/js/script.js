document.addEventListener("DOMContentLoaded", function () {
  ...
  window.addEventListener("scroll", () => {
    window.scrollY > 50
      ? (n.classList.add("bg-white", "shadow-md"),
        n.classList.remove("nav-glass"))
      : (n.classList.remove("bg-white", "shadow-md"),
        n.classList.add("nav-glass"));
  }),
    document.addEventListener("DOMContentLoaded", () => {
      !(function () {
        const e = document.getElementById("new-particles-container");
        for (let t = 0; t < 15; t++) {
          const t = document.createElement("div");
          t.classList.add("new-particle");
          const n = 30 * Math.random() + 10,
            i = 100 * Math.random(),
            l = 100 * Math.random();
          (t.style.width = `${n}px`),
            (t.style.height = `${n}px`),
            (t.style.left = `${i}%`),
            (t.style.top = `${l}%`),
            (t.style.opacity = 0.3 * Math.random() + 0.1),
  ...
  const i = document.querySelectorAll(".tab-btn"),
    l = document.querySelectorAll(".package-card");
  if (i.length > 0 && l.length > 0) {
    function o(e) {
      i.forEach((e) => {
        e.classList.remove("active", "bg-[#756AB6]", "text-white"),
          e.classList.add("bg-white", "text-[#756AB6]", "border");
      }),
      ...
      l.forEach((e) => {
        ...
        "full" === t
          ? document.querySelectorAll(".full-detail").forEach((e) => {
              e.classList.remove("hidden");
            })
          : "exterior" === t
          ? document.querySelectorAll(".exterior-detail").forEach((e) => {
              e.classList.remove("hidden");
            })
          : "interior" === t &&
            document.querySelectorAll(".interior-detail").forEach((e) => {
              e.classList.remove("hidden");
            });
    }
    i.forEach((e) => {
  ...
  r &&
    (window.addEventListener("scroll", function () {
      window.pageYOffset > 300
        ? (r.classList.remove("opacity-0", "invisible"),
          r.classList.add("opacity-100", "visible"))
        : (r.classList.remove("opacity-100", "visible"),
          r.classList.add("opacity-0", "invisible"));
    }),
...
let reviewsInitialized = !1;
function initMap() {
  if (reviewsInitialized) return;
  reviewsInitialized = !0;
  const e = document.getElementById("review-summary"),
    t = document.getElementById("reviews-track");
  e &&
    t &&
    ((e.innerHTML = ""),
    (t.innerHTML = ""),
    document.querySelectorAll(".reviews-link").forEach((e) => e.remove()),
    new google.maps.places.PlacesService(
      ...
      (n, i) => {
        if (i === google.maps.places.PlacesServiceStatus.OK && n) {
          const i = n.user_ratings_total || 0,
            l = n.rating || "N/A";
          if (
            ...
            if (
              ...
              (e.className = "reviews-link text-center mt-8"),
...
function loadReviews() {
  if ("undefined" == typeof google) {
    ...
    (e.src =
      "https://maps.googleapis.com/maps/api/js?key=AIzaSyB6iCfwC2HzjH_RUeB3h8l94lgyZM0_wEg&libraries=places&callback=initMap"),
      (e.async = !0),
      (e.defer = !0),
      (e.id = "google-maps-script"),
      document.head.appendChild(e);
  } else reviewsInitialized || initMap();
}
...
function getPackageInfo(e, t, n) {
  const i = {
    ...
    suv5: {
      full: {
        ...
        gold_full: {
          display: "Gold Full Detail ($210 CAD)",
          price: "210",
  ...
  return i[e]?.[t]?.[n] || i.sedan.full.gold_full;
}
function updateSummary(e, t, n) {
  const i = getPackageInfo(e, t, n);
    ...
    (document.getElementById("summaryVehicle").textContent =
      {
        ...
        xlsuv: "XL SUV",
      }[e] || "Sedan/Coupe (2 Seats)"),
    (document.getElementById("summaryDuration").textContent = i.duration),
    (document.getElementById("summaryTotal").textContent = `$${i.price} CAD`);
}
document.getElementById("review-summary") && loadReviews(),
  document.addEventListener("DOMContentLoaded", function () {
    const e = document.querySelectorAll(".faq-toggle");
    e.length > 0 &&
      e.forEach((t) => {
        t.addEventListener("click", function () {
          const n = this.nextElementSibling,
            i = this.querySelector("svg");
          n &&
            i &&
            ...
            n.classList.contains("hidden") ||
              e.forEach((e) => {
                if (e !== t) {
                  ...
                  t.classList.add("hidden"), n.classList.remove("rotate-180");
                }
              }));
        });
      });
    const t = document.querySelectorAll(".before-after-slider");
    t.length > 0 &&
      t.forEach((e) => {
        const t = e.querySelector('input[type="range"]'),
          n = e.querySelector(".after-image"),
          i = e.querySelector(".slider-handle"),
          l = e.querySelector(".slider-line");
        if (t && n && i && l) {
          function o(e) {
            const o = e || t.value;
            (n.style.clipPath = `inset(0 ${100 - o}% 0 0)`),
              (i.style.left = `${o}%`),
              (l.style.left = `${o}%`);
          }
          o(50), t.addEventListener("input", () => o());
          let r = !1,
            a = 0,
            s = 0;
          e.addEventListener(
            "touchstart",
            (e) => {
              (r = !0), (a = e.touches[0].clientX), (s = a), e.preventDefault();
            },
            { passive: !1 }
          ),
            ...
            e.addEventListener("touchmove", (n) => {
              if (!r) return;
              n.preventDefault();
              const l = n.touches[0].clientX,
                a = l - s;
              s = l;
              const d = e.getBoundingClientRect(),
                c = parseFloat(i.style.left),
                u = Math.min(100, Math.max(0, c + (a / d.width) * 100));
              (t.value = u), o(u);
            }),
            e.addEventListener("mousedown", (e) => {
              (r = !0), (a = e.clientX), (s = a), e.preventDefault();
            }),
    ...
    const n = document.querySelectorAll(".package-btn");
    n.length > 0 &&
      (n.forEach((e) => {
        e.addEventListener("click", function () {
          const e = this.dataset.package;
          n.forEach((e) => {
            e.classList.remove("bg-[#756AB6]", "text-white", "shadow-md"),
              e.classList.add("bg-white", "text-[#756AB6]", "shadow-sm");
          }),
            this.classList.add("bg-[#756AB6]", "text-white", "shadow-md"),
            this.classList.remove("bg-white", "text-[#756AB6]", "shadow-sm"),
            document.querySelectorAll(".process-content").forEach((t) => {
              t && t.classList.toggle("hidden", t.dataset.process !== e);
            });
        });
      }),
      n[0].click());
  ...
  window.addEventListener("load", function () {
    ...
    e && (e.style.display = "none"),
      document.getElementById("review-summary") && loadGoogleMaps();
  }),
  document.addEventListener("DOMContentLoaded", function () {
    const e = new URLSearchParams(window.location.search),
      t = e.get("vehicle") || "sedan",
      n = e.get("service") || "full",
      i = e.get("package") || "gold_full";
    (document.getElementById("displayVehicleType").textContent =
      {
        ...
        suv7: "Pick Up/7-Seat SUV",
        xlsuv: "XL SUV",
      }[t] || "Sedan/Coupe (2 Seats)"),
      (document.getElementById("id_vehicle_type").value = t),
      (document.getElementById("displayServiceType").textContent =
        {
          full: "Full Detail (Interior + Exterior)",
          interior: "Interior Detail",
          exterior: "Exterior Detail",
        }[n] || "Full Detail (Interior + Exterior)"),
      (document.getElementById("id_service_type").value = n);
    const l = getPackageInfo(t, n, i);
    (document.getElementById("displayPackage").textContent = l.display),
      (document.getElementById("id_package_type").value = i.split("_")[0]),
      (document.getElementById("packagePrice").value = l.price),
      updateSummary(t, n, i);
    const o = new Date(),
      r = String(o.getDate()).padStart(2, "0"),
      a = String(o.getMonth() + 1).padStart(2, "0"),
      s = o.getFullYear();
    document.getElementById("id_date").min = `${s}-${a}-${r}`;
  }),
  document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".group.relative").forEach((e) => {
      e.addEventListener("click", function () {
        const e = this.querySelector("img").src,
          t = this.querySelector("h3").textContent,
          n = this.querySelector("p").textContent,
        ...
        (i.className =
          ...
          i.addEventListener("click", function (e) {
            e.target === this && document.body.removeChild(i);
          });
      });
    });
  });
...
if (reelContainer) {
  const e = Array.from(reelContainer.getElementsByClassName("reel")),
    t = document.getElementById("prevReel"),
    n = document.getElementById("nextReel");
  if (e.length > 0 && t && n) {
    let i = 1;
    function updateReels() {
      e.forEach((e, t) => {
        ...
        0 === n
          ? ((e.style.transform = "translateX(0) scale(1)"),
            (e.style.zIndex = "20"),
            l && l.play())
          : -1 === n || 1 === n
          ? ((e.style.transform = "translateX(0) scale(0.75)"),
            (e.style.zIndex = "10"),
            l && l.pause())
          : ((e.style.transform = "translateX(0) scale(0.5)"),
            (e.style.zIndex = "0"),
            l && l.pause());
      });
    }
    function moveReels(t) {
      (i = (i + t + e.length) % e.length), updateReels();
    }
    t.addEventListener("click", () => moveReels(-1)),
      n.addEventListener("click", () => moveReels(1)),
      e.forEach((t) => {
        t.addEventListener("click", () => {
          moveReels(e.indexOf(t) - i);
        });
      }),
      updateReels();
  }
...
if (mobileReels) {
  ...
  function updateMobileCarousel() {
    const e = 100 * -l;
    (mobileReels.style.transform = `translateX(${e}%)`),
      document.querySelectorAll(".mobile-carousel video").forEach((e, t) => {
        e && (t === l ? e.play() : e.pause());
      });
  }
  const r = document.getElementById("mobileNext"),
    a = document.getElementById("mobilePrev");
  r &&
    r.addEventListener("click", () => {
      l < o - 1 && (l++, updateMobileCarousel());
    }),
    a &&
      a.addEventListener("click", () => {
        l > 0 && (l--, updateMobileCarousel());
      });
}
document.querySelectorAll(".mute-btn").forEach((e) => {
  e.addEventListener("click", (e) => {
    const t = e.target.closest("div");
    if (t) {
      const n = t.querySelector("video");
      n &&
        ((n.muted = !n.muted), (e.target.textContent = n.muted ? "🔇" : "🔊"));
    }
  });
}),
  document.addEventListener("DOMContentLoaded", function () {
    const e = document.getElementById("confetti-container"),
      t = ["#AC87C5", "#756AB6", "#E0E7FF", "#F5F3FF"];
    for (let n = 0; n < 50; n++) {
      const n = document.createElement("div");
      (n.className = "confetti"),
        (n.style.left = 100 * Math.random() + "vw"),
        (n.style.animationDelay = 5 * Math.random() + "s"),
        (n.style.width = 8 * Math.random() + 5 + "px"),
        (n.style.height = n.style.width),
        (n.style.backgroundColor = t[Math.floor(Math.random() * t.length)]),
        (n.style.opacity = 0.5 * Math.random() + 0.3),
        e.appendChild(n);
    }
  });
